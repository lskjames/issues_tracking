/* Boostmine Theme version 2.0, Copyright (C) 2017-2019 Luis Blasco www.bestredminetheme.com */
...
$(document).ready(function() {	
	...
	$('#query_form_with_buttons .collapsible, #query_form .collapsible').click(function(){
		...
		if (!$("#query_form_with_buttons .collapsible").not(".collapsed").length) {			
			...
			$('#query_form_with_buttons a').css("display", "inline-block");
...
$(window).bind("load", function() {		
	...
	var setupCheckboxes = function() {
		...
		$('input[type=checkbox]').each(function() {	
			...
			if ( $this.parents('#helpdesk_send_response').length || $this.parents('#tab-content-activities').length) {				
			} else {
...
//console.log(container4.has(e.target).length);
	...
	if($('.controller-gantts.action-show, .controller-agile_boards.action-index, .controller-custom_fields.action-edit, .controller-custom_fields.action-new, .controller-roles.action-edit, .controller-trackers.action-edit, .controller-settings.action-index').length == 0 ){
		
		setupCheckboxes();
		
		if($('.controller-settings.action-plugin').length == 0 ){
			setupRadioboxes();
		}
